...
async function cleanupWebtec() {
    try {
        ...
        for (const template of res.rows) {
            ...
            // Ensure SEGURIDAD OPERATIVA exists
        ...
        console.log('\nWEBTEC checklists cleaned up successfully.');
        process.exit(0);
    } catch (error) {
        console.error('Error cleaning up WEBTEC checklists:', error);
        process.exit(1);
    }
}
